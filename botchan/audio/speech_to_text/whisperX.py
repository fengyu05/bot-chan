...
class WhisperX(Singleton, SpeechToText):
        ...
        if use == "local":
            ...
            if OPENCC:
                ...
                assert OPENCC in [
                    "t2s",
                    "s2t",
                ], 'OPENCC should be either "t2s" or "s2t"'
    ...
    def _transcribe_local(
        ...
        segments = [
            DiarizedSingleSegment(**seg, speaker="") for seg in result["segments"]
        ]
        ...
        if DIARIZATION and diarization:
            ...
            if speaker_audio_samples:
                speaker_audios = {
                    id: self.get_audio(ab, platform)
                    for id, ab in speaker_audio_samples.items()
                }
